...
const options = require('yargs')
  .usage('Usage: $0 --local local_dir1 [--local local_dir2] [--remote https://example.com] [--redirect /one /another]')
  ...
  .array('redirect')
  .describe('redirect', 'Use a HTTP redirect from one path to another')
  ...
  .requiresArg(['l', 'p', 'a', 'r', 'x', 'redirect'])
  .version()
...
if (options.redirect) addAliases(options.redirect)
...
if (options.remote) app.use(forwardTo(options.remote))
...
function addAliases(aliases) {
  // TODO: validate that these are actually pairs!
  const pairs = _.chunk(aliases, 2)
  pairs.forEach(function addAlias([from, to]) {
    app.get(from, rewrite(to))
  })

  function rewrite(to) {
    return function (req, res, next) {
      console.log(`rewriting '${req.url}' to '${to}'`)
      req.url = to
      next()
    }
  }
